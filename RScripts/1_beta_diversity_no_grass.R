# Beta-diversity without grasses ####
...
# here: but without pollen of Poaceae or Cyperaceae (grasses)


# sdt <- sdt[,-which(colnames(sdt) %in% c("Poaceae", "Cyperaceae", "Cyperus"))] # remove grasses
# sdt <- subset(sdt, select = -c("Poaceae", "Cyperaceae", "Cyperus"))
...
# rename all objects to _NG
readme_NG             <- readme
list_all_NG           <- list_all
list_cons_NG          <- list_cons
list_cons_bin_NG      <- list_cons_bin
list_bold_NG          <- list_bold
list_bold_bin_NG      <- list_bold_bin

dat_NG                <- dat
dat_con_NG            <- dat_con
dat_global_NG         <- dat_global
dat_global_con_NG     <- dat_global_con 
dat_island_group_NG   <- dat_island_group
dat_archipelago_NG    <- dat_archipelago
dat_biogeo_NG         <- dat_biogeo

master_bold_df_NG     <- master_bold_df
master_cons_df_NG     <- master_cons_df
stand_master_NG       <- stand_master



# save data ----
save(readme_NG, # metadata
     list_all_NG, # all data
     list_cons_NG, list_cons_bin_NG, # str 1, unbinned and binned
     list_bold_NG, list_bold_bin_NG, # str 2, unbinned and binned
     
     dat_NG, dat_con_NG, # island-island comparison for str 1 and 2, for all three indices
     dat_global_NG, dat_global_con_NG, # dataframe with all island-island comparisons and geographic categorization
     dat_island_group_NG, # data classified by island group
     dat_archipelago_NG, # data classified by archipelago
     dat_biogeo_NG, # data classified by biogeographic region
     
     master_bold_df_NG, master_cons_df_NG, # alls islands, all pollen data, all time steps
     stand_master_NG, # taxonomic standardization master table
     file = "./Data/R_products/similarity_islands_NG.RData")
